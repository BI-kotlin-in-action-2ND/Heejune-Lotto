...
class LottoTicketsTest : FunSpec({
    ...
    context("LottoTickets operations") {
        val ticket1 = LottoTicket(listOf(1, 2, 3, 4, 5, 6))
        val ticket2 = LottoTicket(listOf(7, 8, 9, 10, 11, 12))
        ...
        test("matchAll should correctly match numbers against WinningLotto") {
            val winningNumbers = WinningLotto(listOf(1, 2, 3, 7, 8, 9), 10)
        ...
        test("plus operator should combine LottoTickets instances") {
            val ticket3 = LottoTicket(listOf(13, 14, 15, 16, 17, 18))
